...
interface IFormComponentProps extends FormComponentProps {
  ...
  current: IComment;
  onAdd: (args: IComment) => void;
  onEdit: (args: IComment) => void;
  onModalVisible: (args?: boolean) => void;
}

const CreateForm: React.FC<IFormComponentProps> = props => {
  const { modalVisible, form, current, onAdd, onEdit, onModalVisible } = props;

  const handleOk = () => {
    form.validateFields((err, fieldsValue: IComment) => {
      if (err) return;
      if (fieldsValue.id) {
        onEdit(fieldsValue);
      } else {
...
export default Form.create<IFormComponentProps>()(CreateForm);
